...
export namespace Types {
    ...
    export function isIndexedAccess(o: unknown): o is JSONOutput.IndexedAccessType {
...
export function parseType(t: JSONOutput.Type | JSONOutput.SomeType | string): string {
    ...
    if (Types.isConditional(t)) return `${parseType(t.checkType)} extends ${parseType(t.extendsType)} ? ${parseType(t.trueType)} : ${parseType(t.falseType)}`

    if (Types.isTuple(t)) return `[${(t.elements ?? []).map(parseType).join(", ")}]`
    ...
    if (Types.isUnion(t)) return t.types.map(parseType).filter(c => c && c.trim().length > 0).join(" |")

    if (Types.isReference(t)) return t.name + (t.typeArguments ? `<${t.typeArguments.map(parseType).join(', ')}>` : "")

    if (Types.isPredicate(t)) return (t.asserts ? "asserts " : "") + t.name + (t.targetType ? ` is ${parseType(t.targetType)}` : "") 

    if (Types.isInferred(t) || Types.isUnknown(t) || Types.isIntrinsic(t)) return t.name

    if (Types.isIndexedAccess(t)) return `${parseType(t.objectType)}[${parseType(t.indexType)}]`
