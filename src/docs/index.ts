...
import type { DeclarationReflection, LiteralType, ParameterReflection, SignatureReflection, SourceReference } from "typedoc";
import { getVarNames, parseType, Types } from "../utils/index";
import type { Class, Config, Constructor, Event, External, Interface, Item, Member, Meta, Method, Param, Typedef, VarType } from "../utils/types";
...
export class DocumentedItem<T = DeclarationReflection | Item> {
    ...
    serialize() {
        try {
            ...
            err.message = `Error while serializing ${this.toString()}: ${err.message}`
        }
    }
    ...
    toString() {
		...
		if (data.id || data.name) return `${data.id ?? data.name} (${this.constructor.name})`;
...
export class DocumentedClass extends DocumentedItem<Class | DeclarationReflection> {
	...
	add(item: DocumentedConstructor | DocumentedEvent | DocumentedMember | DocumentedMethod) {
